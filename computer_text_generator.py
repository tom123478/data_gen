...
def _generate_horizontal_text_stroke(text, font, text_color, font_size, space_width, fit, strokewidth, strokefill):
    ...
    image_font = ImageFont.truetype(font=font, size=font_size)
    words = text.split(' ')
    space_width = (image_font.getsize(' ')[0]+gap) * space_width 

    words_width = [image_font.getsize(w)[0] for w in words]
    text_width =  sum(words_width) + int(space_width) * (len(words) - 1) + gap*2
    text_height = max([image_font.getsize(w)[1] for w in words]) + gap*3
